...
# jump only available when pressing direction keys
			if Input.is_action_just_pressed("jump") and (Input.is_action_pressed("move-right") or Input.is_action_pressed("move-left")):
...
# handles jumps in air or from wall
		...
		if not (Input.is_action_pressed("move-right") or Input.is_action_pressed("move-left")) and is_on_wall():
			pass
